...
\affil*[1]{\orgdiv{Computing and Software Department}, \orgname{McMaster University}, \orgaddress{\street{1280 Main Street West}, \city{Hamilton}, \postcode{L8S 4K1}, \state{Ontario}, \country{Canada}}}

\affil[2]{\orgdiv{Electrical and Computer Engineering Department}, \orgname{McMaster University}, \orgaddress{\street{1280 Main Street West}, \city{Hamilton}, \postcode{L8S 4K1}, \state{Ontario}, \country{Canada}}}
...
quantitive data, we ranked the MI software.  The top five are \textit{3D
Slicer}, \textit{ImageJ}, \textit{Fiji}, \textit{OHIF Viewer}, and
\textit{ParaView}.  This is consistent with the community's, with four of
these also appearing in the top five using GitHub metrics (stars-per-year).  
In general, the quality and quantity of documentation present in the
project correlates quite well with its popularity.
%We observed 88\% of the documentation artifacts
...
and provide recommendations for future development.
...
%research software project \cite{SmithEtAl2021, SmithAndMichalski2022}. We
%particularly focus on \emph{software artifactcs}, i.e.
%the documents, scripts and code that constitutes a software development
...
\item [RQ\refstepcounter{rqnum}\therqnum \label{RQ_HighestQuality}:] Establish
quantitative measurements of those project's development practices. From
these measures, which of those projects follow best practices?
...
We only cover MI visualization software.  We exclude
other categories of MI software, including
...
and Communication).  We also exclude Statistical Analysis and Image-based
Physiological Modelling and Feature Extraction, Classification, and
Interpretation. Software that provides MI support functions is also out of
...
We have a standard set of questions designed to assess the qualities of any
research software project~\cite{SmithEtAl2021, SmithAndMichalski2022}.
This has been applied to MI software and Lattice Boltzmann Solvers
\cite{SmithEtAl2024}.  This builds off prior work to assess the
...
\cite{smith2018statistical}.  We maintain the previous constraint that the
work load for measuring a given domain should take around one person-month's
worth of effort ($20$ working days at $8$ person-hours per day).

We identify a list of potential packages (through online searches) which is
then filtered and vetted by a domain expert. We aim for roughly $30$
packages. For each remaining package, we measure its qualities by filling in a
grading template~\cite{SmithEtAl2021}.  This data is used to rank the projects
with AHP.  Further details on interaction with the domain expert, software
qualities, grading the software and AHP are summarizes below and in longer form
in~\cite{SmithEtAl2024_MI_SOP}.

\subsubsection{Domain Expert} \label{sec_vet_software_list}

The Domain Expert vets the proposed list. For too many domains, there are
online resources that are wildly inaccurate, which only an expert can correct.
(Details of our domain expert removed for double-blind).
...
We use an existing template~\cite{SmithEtAl2021} that is designed to measure
the aforementioned qualities. To stay within our given measurement time frame,
each package gets up to five hours of time.  Project developers can be
contacted for help regarding installation, if necessary, but we impose a cap of
about two hours on the installation process.
Figure~\ref{fg_grading_template_example} shows an excerpt of the measurement
spreadsheet.  It includes a column for each measured software package.
% \cite{Dong2021-Data} provides the full set of measurement data.
(The full data is available on Mendeley; link will be provided after
refereeing.)
...
%string, a date, a set of strings, etc. 
The grader, after answering questions for each quality assigned an overall
...
measure.  For example, we do not do user studies for measuring usability.
Instead, we are look signs that the developers considered usability.
...
Developed by Saaty in the 1970s, AHP is widely used to 
analyze multiple criteria decisions~\cite{VaidyaEtAl2006}. AHP organizes
...
We initially identified 48 candidate software projects from
the literature \cite{Bjorn2017, Bruhschwein2019, Haak2015}, on-line articles
\cite{Emms2019, Hasan2020, Mu2019}, and forum discussions \cite{Samala2014}.
Then we filtered as follows:
...
\item Focused on MI software that provides visualization functions.
We removed seven packages
that were toolkits or libraries, such as \textit{VTK}, \textit{ITK}, and
\textit{dcm4che}, and another three that were for PACS.
...
The Domain Expert provided a list 12 software packages.
We found 6 packages were on both lists: \textit{3D
Slicer}, \textit{Horos}, \textit{ImageJ}, \textit{Fiji}, \textit{MRIcron} 
(we use its descendant \textit{MRIcroGL}) and \textit{Mango} (we
use the web version \textit{Papaya}).  The remaining six packages were on our
out-of-scope list. The Domain Expert agreed with our final choice of 29
packages -- see Table~\ref{tab_final_list}. This contains summary data
collected in the year 2020. 

The projects are sorted in descending order of lines of code.
We found the initial release dates (Rlsd) for most
...
techniques to improve correctness, and had better documentation to witness
this. For instance, we looked for evidence of unit testing, and only evidence
for about half of the projects. We identified five projects using CI/CD
...
Figure~\ref{fg_reliability_scores} shows the our results.  We were able to
follow the steps in the tutorials that existed (seven packages had them.)
However
...
The packages with higher scores elegantly handled unexpected or unanticipated
...
\textit{ImageJ}, \textit{Fiji} included all three documents 
(see Table~\ref{tab_maintainability_docs} the full data).  Moreover,
...
API documents (see Table~\ref{tab_maintainability_docs}) also help maintainability,
which explains some of the scores shown in
Figure~\ref{fg_reusability_scores}. We have assumed that smaller code files are
likely more reusable -- see
Table \ref{tab_loc_per_file} for the details.
...
popularity of projects as judged by the scientific community, we compare our
ranking (from Section~\ref{ch_results}) with the community ratings on GitHub, as
shown by GitHub stars, number of forks, and number of people watching the
projects.
...
\cite{IEEE1998, NASA1989, RobertsonAndRobertson1999Vol}.
...
for developers on how they should treat one another \cite{TouraniEtAl2017}.
Three popular codes of conduct are \cite{TouraniEtAl2017}:
...
conduct can improve the participation of women \cite{SinghEtAl2021}.
...
practices and dangers to avoid \cite{Carty2020}.  Three sample style guides are:
...
roadmaps cover the following: \begin{inparaenum}[i)]
	...
	\item How can we get there? \end{inparaenum} \cite{PhaalEtAl2005}. A product
roadmap provides the following benefits: continuity of purpose, facilitation of
collaboration, and assistance with prioritization \cite{Pichler2012}.
...
documentation include representation of the system design and class design using
Unified Modelling Language class diagrams \cite{Gamma1995}, or rigorous
documentation of the system design following the template for a Module Guide
(MG) \cite{ParnasEtAl1984}.  An MG organizes the modules in a hierarchy by their
secrets.
...
Boltzmann solvers \cite{SmithEtAl2024}, except LBM software is more likely to
...
engineering recommendations \cite{Storer2017}.  Johanson and Hasselbring observe
that the state-of-the-practice for research software in industry and academia
does not incorporate state-of-the-art SE tools and methods
\cite{JohansonAndHasselbring2018}.  This causes sustainability and reliability
problems \cite{FaulkEtAl2009}. Rather than benefit from capturing and reusing
previous knowledge, projects waste time and energy ``reinventing the wheel''
\cite{deSouzaEtAl2019}.

\subsection{Threats to Validity} \label{sec_threats_to_validity}

Our categories for threat to validity come from an analysis of software
engineering secondary studies by Ampatzoglou et al.\ \cite{AmpatzoglouEtAl2019},
where a secondary study analyzes the data from a set of primary studies.
Ampatzoglou et al.\ is appropriate because a common example of a secondary study
is a systematic literature review. Our methodology is a systematic software
review --- the primary studies are the software packages, and our work collects
and analyzes these primary studies.  We identified similar threats to validity
in our assessment of the state of the practice of Lattice Boltzmann Solvers
\cite{SmithEtAl2024}.
...
reproducible.  In previous work \cite{SmithEtAl2016} we graded five software
...
to measure \cite{RunesonAndHost2009}. Our construct threats are often related to
how we assume our measurements influences the various software qualities, as
summarized in Section~\ref{sec_grading_software}.  Specifically, our construct
validity related threats include the following:
...
information outside their repos, possibly in journal papers or technical
reports.
\end{itemize}
...
missed any highly popular MI examples.  Therefore, the bulk of our validity
concerns are internal (Section~\ref{Sec_InternalValidity}).  However, our hope
is that the trends observed, and the lessons learned for MI software can be
applied to other research software.  With that in mind we identified the
following threat to external validity: We cannot generalize our results if the
development of MI software is fundamentally different from other research
software.
...
understanding current practice by answering four research questions
...
with the help of the Domain Expert selected 29 of them for our final list. 
...
unable to install (\textit{dwv} and \textit{Dicom Viewer}). Our findings suggest
that the community's top ranked packages tend to be those that incorporate best
practices.
...
When it comes to recommended software artifacts, the state of open source MI
projects is healthy, with our surveyed examples showing 88\% of the
documentation artifacts recommended by research software development guidelines
(Section~\ref{Sec_CompareArtifacts}).  However, we did notice areas where
practice seems to lag behind the research software development guidelines.  For
instance, the guidelines recommend three artifacts that were not observed:
uninstall instructions, test plans, and requirements documentation. We observed
the following recommended artifacts, but only rarely: contributing file,
developer code of conduct, code style guidelines, product roadmap, design
documentation, and API documentation (Section~\ref{Sec_CompareArtifacts}).
Future MI projects may wish to consider incorporating more artifacts to
potentially improve software quality.  A deeper understanding of the needs of
the MI community will require data beyond what is available in open source
repositories.  Future work should involve interviewing MI developers to more
deeply understand their software development pain points.
