...
29 projects from 48 candidates and assessed 9 software qualities
(installability, correctness/ verifiability, reliability, robustness,
usability, maintainability, reusability, understandability, and
visibility/transparency) by answering 108 questions for each. Using the
Analytic Hierarchy Process (AHP) on the quantitative data, we ranked the MI
software.  The top five are \textit{3D Slicer}, \textit{ImageJ}, \textit{Fiji},
\textit{OHIF Viewer}, and \textit{ParaView}.  This is consistent with the
community's view, with four of these also appearing in the top five using
GitHub metrics (stars-per-year).  The quality and quantity of documentation
present in a project correlates quite well with its popularity.  Generally, MI
software is in a healthy state: in the repositories
we observed 88\% of the documentation artifacts recommended by research
software development guidelines, 100\% of MI projects use version control
tools, and developers appear to use the common quasi-agile research software
development process. However, the current state of the practice deviates from
existing guidelines as some recommended artifacts are rarely present
(like a test plan, requirements specification, code of conduct, and code style
guidelines), low usage of continuous integration (17\% of the projects), low
use of unit testing (about 50\% of projects), and room for improvement with
documentation. From developper interviews , we identified 7 concerns: lack
of development time, lack of funding, technology hurdles, correctness,
usability, maintainability, and reproducibility. We recommend:
increase effort on documentation,
increase testing by enriching datasets, increase continuous integration usage,
move to web applications, employ linters, use peer reviews, and design for
change.
...
understand the impact of the often cited gap between recommended
...
Our investigation is based on the following research questions:
\begin{enumerate}
...
We weighted all qualities equally, which is not the likely (implicit) weight
that users implicitly use. To properly assess this would require a broad user
study.  Furthermore our measures of popularity are only \emph{proxies} which
...
factors than just quality that influence the popularity of ``consumer''
products.
...
(C), uncommonly (U) or rarely (R).  We did our best to interpret the meaning of
each artifact consistently between guidelines and specific MI software, but the
...
significant variation between projects~\cite{PranaEtAl2018}.  Some content is
reasonably consistent, with 97\% of README files contain at least one section
describing the `What' of the repository and 89\% offering some `How' content,
other categories are more variable.  For instance, information on
...
Note that ``popularity'' in Table~\ref{Tbl_Guidelines} does not imply that these
...
Developers use software tools for user support, version control (VCS),
...
from other VCS.  This represents a considerable uptake since its rare use in
2006~\cite{Wilson2006}, which matches the uptake in other communities: 10 years
ago Nguyen et al~\cite{Nguyen-HoanEtAl2010} estimated that only 50\% of
research software projects used version control, and by 2018 this had jumped to
over 80\%~\cite{AlNoamanyAndBorghi2018} with many communities~\cite{Smith2018}
reaching 100\% already in 2018. In 2022, version control use sits at over
...
A recent survey~\cite{CarverEtAl2022} of practioners (rather than projects)
...
\paragraph{Project management} Two types of tools were mentionned:
\begin{inparaenum}[i)]
...
Based on~\cite{JungEtAl2022}, tool use for MI
software is similar to tool use for ocean modelling software.
Both use tools for editing, compiling, code management, testing, building, and
project management, but differ
in the latter's use of Kanban boards for project management.
...
followed agile or alike, two teams  followed waterfall or alike, and three
teams explicitly stated that their process was undefined or self-directed.
...
No interviewee mentioned a strictly defined project management process. 
...
We draw out the point points the points experienced by MI
software developers, compare them to those of general research software
developers, and what practices are used to address these pain points.

Interviews identified the following issues:
\begin{enumerate*}
\item lack of time and funding,
...
there is an ongoing struggle to attract funding and, even harder, maintain
research software. This is a common complaint%
\cite{GewaltigAndCannon2012, Goble2014, KaterbowAndFeulner2018, SmithEtAl2024}.
As software output is not always counted when judging the academic excellence
of applicants, which forces researchers who write software to spend extra
time on publicity~\cite{WieseEtAl2019}. In some domains, like biology%
\cite{HowisonAndBullard2016}, software is infrequently cited even when used
pervasively. In other words, there is a lack of a formal reward system for
research software~\cite{PintoEtAl2018}.

\item[P\refstepcounter{pnum}\thepnum \label{P_TechnologyHurdles}:]
\textbf{Technology Hurdles:} Developers mentioned: hard to keep up with changes
in OS and libraries, difficult to transfer to new technologies, hard to support
multiple OSes, and hard to support lower-end
...
Example sources of complexity are: Interviewees identified multiple threats to
the large variety of data formats, complicated data standards, differing
outputs between medical imaging machines, and the addition of (non-viewing
related) functionality.  Other identified threats to correctness:
...
lack expertize in their domain to be able to adequately use the software
\cite{WieseEtAl2019}. 
...
In general, solutions need to be more systemic, and we are seeing the rise
...
building web applications, and another team was considering it too.

\item[P\ref{P_Correctness}:] \textbf{Correctness:}
Testing was the most often strategy for ensuring correctness, and teams
...
Synthesizing the feedback from the interviewees, it is clear that increased
connections between the development team and medical professionals/institutions
could improve testing effectiveness and thus increase correctness.
...
\section{Software Qualities}\label{sec:sq}

The structured interviews also included an active discussion of software
qualities, and in particular maintainability and reproducibility. Note that
these did not come up as ``pain points''.
%\cite{SmithEtAl2021}.  
The developers discussed their approaches for improving these qualities, which
we now relay.
...
The main strategy is to reduce code duplication by using properly structured
...
An important precondition for reproducibity is increased and better
documentation (which also helps with many of the afore mentioned pain points).
