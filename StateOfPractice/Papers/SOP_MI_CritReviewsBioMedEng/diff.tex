...
%DIF DEL OrigSubmissionSOP_MI_OSR.tex   Sun Nov 24 14:42:43 2024
%DIF ADD SOP_MI_OSR.tex                 Sat Jan 25 22:17:53 2025
%DIF < \documentclass[doubleblind,12pt, 3p, times]{elsarticle}
%DIF -------
%\documentclass[doubleblind,12pt, 3p, times]{elsarticle} %DIF > 
\documentclass[12pt, 3p, times]{elsarticle} %DIF > 
%DIF -------
...
%DIF 20a21-22
 %DIF > 
\usepackage[tableposition=top]{caption} %to space the caption properly %DIF > 
%DIF -------
...
\DIFdelbegin \subsection{\DIFdel{Research Questions}} %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec_motivation}
%DIFDELCMD < %%%
\DIFdelend %DIF > \subsection{Research Questions} \label{sec_motivation}
...
How does the use of principles, processes, and methodologies compare between MI software and research software in general? (Section~\ref{Sec_CompareMethodologies})
...
\DIFdelbegin \subsection{\DIFdel{Scope}} %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec_scope}
%DIFDELCMD < %%%
\DIFdelend %DIF > \subsection{Scope} \label{sec_scope}
...
\DIFdelbegin \subsection{\DIFdel{Methodology}} %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Methodology}} \DIFaddend \label{SecMethodology}

We have a standard set of questions designed to assess the qualities of any
research software project\DIFdelbegin \DIFdel{.  }%DIFDELCMD < [%%%
\DIFdel{blind review redacted details on the author's
previous studies using the same methodology.  }%DIFDELCMD < ]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{SmithEtAl2021, SmithAndMichalski2022}}\hskip0pt%DIFAUXCMD
. We build
off prior work to assess the state of the practice for such domains as Lattice
Boltzmann Solvers \mbox{%DIFAUXCMD
\cite{SmithEtAl2024}}\hskip0pt%DIFAUXCMD
, Geographic Information Systems
\mbox{%DIFAUXCMD
\cite{smith2018state}}\hskip0pt%DIFAUXCMD
, Mesh Generators \mbox{%DIFAUXCMD
\cite{smith2016state}}\hskip0pt%DIFAUXCMD
, Seismology
software \mbox{%DIFAUXCMD
\cite{Smith2018Seismology}}\hskip0pt%DIFAUXCMD
, and Statistical software for psychology
\mbox{%DIFAUXCMD
\cite{smith2018statistical}}\hskip0pt%DIFAUXCMD
.  }\DIFaddend %% DOUBLE BLIND 
%DIF <  ~\cite{SmithEtAl2021, SmithAndMichalski2022}.  This has
%DIF <  been applied to MI software and Lattice Boltzmann Solvers \cite{SmithEtAl2024}.
%DIF <  This builds off prior work to assess the state of the practice for such domains
%DIF <  as Geographic Information Systems \cite{smith2018state}, Mesh Generators
%DIF <  \cite{smith2016state}, Seismology software \cite{Smith2018Seismology}, and
%DIF <  Statistical software for psychology \cite{smith2018statistical}.  
We maintain the previous
constraint that the work load for measuring a given domain should take around
one person-month's worth of effort ($20$ working days at $8$ person-hours per
day).
...
template\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for double blind}%DIFDELCMD < ]%%%
\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{SmithEtAl2021}}\hskip0pt%DIFAUXCMD
}\DIFaddend .    %%DOUBLE BLIND
%DIF < ~\cite{SmithEtAl2021}.  
...
AHP below and in longer form in \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{redacted for double blind}%DIFDELCMD < ]%%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{Smith et al.\ (2024)~\mbox{%DIFAUXCMD
\cite{SmithEtAl2024_MI_SOP}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend %% DOUBLE BLIND
%DIF < Smith et al.\ (2024)~\cite{SmithEtAl2024_MI_SOP}.

\DIFdelbegin \subsubsection{\DIFdel{Domain Expert}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Domain Expert}} \DIFaddend \label{sec_vet_software_list}

The Domain Expert vets the proposed list because online resources can be
inaccurate.  The expert also vets the AHP ranking.  Our Domain Expert \DIFdelbegin \DIFdel{is }%DIFDELCMD < [%%%
\DIFdel{details of our domain expert removed for double-blind}%DIFDELCMD < ]%%%
\DIFdel{.%DIF < %DOUBLE BLIND %our Domain Expert (and paper co-author) is Dr.\ Michael
%DIF < Noseworthy, Professor of Electrical and Computer Engineering at McMaster
%DIF < University, Co-Director of the McMaster School of Biomedical Engineering, and
%DIF < Director of Medical Imaging Physics and Engineering at St.\ Joseph's
%DIF < Healthcare, Hamilton, Ontario, Canada.  
}\DIFdelend \DIFaddbegin \DIFadd{(and
paper co-author) is Dr.\ Michael Noseworthy, Professor of Electrical and
Computer Engineering at McMaster University, Co-Director of the McMaster School
of Biomedical Engineering, and Director of Medical Imaging Physics and
Engineering at St.\ Joseph's Healthcare, Hamilton, Ontario, Canada.  %DIF > %DOUBLE BLIND 
}\DIFaddend 
...
\DIFdelbegin \subsubsection{\DIFdel{Software Qualities}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Software Qualities}} \DIFaddend \label{sec_software_quality}
...
\DIFdelbegin \subsubsection{\DIFdel{Grading}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Grading}} \DIFaddend \label{sec_grading_software}

We use an existing template \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted}%DIFDELCMD < ]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{SmithEtAl2021} }\hskip0pt%DIFAUXCMD
}\DIFaddend %% DOUBLE BLIND
...
hours on the installation process.  \DIFaddbegin \DIFadd{We explicitly note any software that cannot be installed, but for completeness we still measure it for the remaining qualities, since only reliability and robustness require running the software. }\DIFaddend Figure~\ref{fg_grading_template_example}
shows an excerpt of the measurement spreadsheet.  The rows are the measures and
the columns correspond to the software packages.  \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{All datacan be found on
Mendeley; link will be provided after refereeing.   }%DIFDELCMD < ] 
%DIFDELCMD < %%%
%DIF <  \cite{Dong2021-Data}
%DIF < provides the full set of measurement data.   %%DOUBLE BLIND
\DIFdelend \DIFaddbegin \DIFadd{Dong (2021)~\mbox{%DIFAUXCMD
\cite{Dong2021-Data}
}\hskip0pt%DIFAUXCMD
provides the full set of measurement data.   %DIF > %DOUBLE BLIND
}\DIFaddend 
...
details on quality measurement are provided in \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{redacted for double blind}%DIFDELCMD < ]%%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{Smith et al.\ (2024) \mbox{%DIFAUXCMD
\cite{SmithEtAl2024_MI_SOP}}\hskip0pt%DIFAUXCMD
. 
...
\DIFdelbegin \subsubsection{\DIFdel{Analytic Hierarchy Process (AHP)}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Analytic Hierarchy Process (AHP)}} \DIFaddend \label{sec_AHP}
...
\DIFdelbegin \subsubsection{\DIFdel{Interview Methods}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Interview Methods}} \DIFaddend \label{sec_interview_methods}
...
\DIFdel{citation redacted for double blind}%DIFDELCMD < ]%%%
\DIFdel{. %DIF < Smith et al. \cite{SmithEtAl2021}. 
}\DIFdelend \DIFaddbegin \DIFadd{Smith et al. \mbox{%DIFAUXCMD
\cite{SmithEtAl2021}}\hskip0pt%DIFAUXCMD
. %DIF > %DOUBLE BLIND
}\DIFaddend Some questions are about the background of the software, the development teams,
...
\begin{table*}[ht!]
\DIFaddbeginFL \caption{\DIFaddFL{Final software list (sorted in descending order of the number of Lines
Of Code (LOC))}}
...
{%DIFAUXCMD
\DIFdelFL{Final software list (sorted in descending order of the number of Lines
Of Code (LOC))}}
%DIFAUXCMD
\DIFdelendFL \label{tab_final_list}
\end{table*}
...
\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{removed for blind review}%DIFDELCMD < ]
%DIFDELCMD < %%%
%DIF < \href{https://data.mendeley.com/datasets/k3pcdvdzj2/1} {Mendeley Data}.  
\DIFdelend \DIFaddbegin \href{https://data.mendeley.com/datasets/k3pcdvdzj2/1} {\DIFadd{Mendeley Data}}\DIFadd{.  
}\DIFaddend %% DOUBLE BLIND
...
equally important.  \DIFaddbegin \DIFadd{(Alternative priority schemes can be investigated by changing the
...
{\DIFadd{AHP\_Template.xlsx}} \DIFadd{available on GitHub.) }\DIFaddend Figure~\ref{fg_overall_scores} shows
...
details of how the projects compare for each quality can be found in \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation
redacted for blind review}%DIFDELCMD < ]%%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{Smith et
al.~\mbox{%DIFAUXCMD
\cite{SmithEtAl2024_MI_SOP}}\hskip0pt%DIFAUXCMD
. }\DIFaddend %% DOUBLE BLIND
%DIF < Smith et al.~\cite{SmithEtAl2024_MI_SOP}
\DIFaddbegin 

\DIFaddend The highlights are as follows:
...
\begin{table*}[ht!]
\DIFaddbeginFL \caption{\DIFaddFL{Number of files and lines (by reusability scores)}}
\DIFaddendFL \centering
\begin{tabular}{lllll}
\toprule
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Number of files and lines (by reusability scores)}}
%DIFAUXCMD
\DIFdelendFL \label{tab_loc_per_file}
\end{table*}

\section{Comparison to Community Ranking} \label{Sec_VsCommunityRanking}

We use GitHub stars, number of forks and
number of people watching the projects as proxies for community ranking.  \DIFaddbegin \DIFadd{We considered also comparing software citations, but this measure of popularity would be unreliable since software is infrequently and inconsistently cited~\mbox{%DIFAUXCMD
\cite{SmithEtAl2016-softcite}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend Table~\ref{tab_ranking_vs_GitHub} show the statistics for \DIFaddbegin \DIFadd{GitHub }\DIFaddend data collected in July
2021.
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{Besides the installation problem, another possible reason for discrepancies between our ranking and GitHub popularity is that we }\DIFaddend weighted all qualities equally, which is not the likely the same weighting
...
\begin{table*}[ht!]
\DIFaddbeginFL \caption{\DIFaddFL{Software ranking by our methodology versus the community (Comm.)\
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Software ranking by our methodology versus the community (Comm.)\
ranking using GitHub metrics (Sorted in descending order of community
popularity, as estimated by the number of new stars per year)}}
%DIFAUXCMD
\DIFdelendFL \label{tab_ranking_vs_GitHub}
\end{table*}
\endgroup
...
\begin{table*}[ht!]
\DIFaddbeginFL \caption{\DIFaddFL{Comparison of Recommended Artifacts in Software Development Guidelines
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparison of Recommended Artifacts in Software Development Guidelines
to Artifacts in MI Projects (C for Common, U for Uncommon and R for Rare)}}
%DIFAUXCMD
\DIFdelendFL \label{Tbl_Guidelines}
\end{center}
\end{table*}
...
\begin{table*}[ht!]
    \DIFaddbeginFL \caption{\DIFaddFL{Software Artifacts Found in MI Packages, Classified by Their Number
    of Occurrences}}
    \DIFaddendFL \begin{center}
    ...
    \DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Software Artifacts Found in MI Packages, Classified by Their Number
    of Occurrences}}
    %DIFAUXCMD
\DIFdelendFL \label{artifactspresent}
    \end{center}
\end{table*}
...
We interview \DIFdelbegin \DIFdel{data}\DIFdelend \DIFaddbegin \DIFadd{developers}\DIFaddend , as described in Section~\ref{sec_interview_methods}\DIFaddbegin \DIFadd{, }\DIFaddend to answer \rqref{RQ_CompareToolsProjMngmnt} (tool usage),
...
found in \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind review}%DIFDELCMD < ]%%%
\DIFdel{. %DIF < \cite{Dong2021}.
}\DIFdelend \DIFaddbegin \DIFadd{Dong (2021)~\mbox{%DIFAUXCMD
\cite{Dong2021}}\hskip0pt%DIFAUXCMD
. %DIF > %DOUBLE BLIND
}\DIFaddend 
...
\begin{table}[ht!]
\DIFaddbeginFL \caption{\label{tab_user_support_model}\DIFaddFL{User support models by number of projects}}
\DIFaddendFL \centering
\begin{tabular}{lc}
\toprule
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tab_user_support_model}%%%
\DIFdelFL{User support models by number of projects}}
%DIFAUXCMD
...
(LBM) software\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind review}%DIFDELCMD < ]%%%
\DIFdel{. %DIF < \cite{Michalski2021}.
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Michalski2021}}\hskip0pt%DIFAUXCMD
. %DIF > %DOUBLE BLIND
}\DIFaddend This contrasts with the high frequency of guidelines recommending continuous
integration
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{BrettEtAl2021, Brown2015, ThielEtAl2020, Zadka2018, vanGompelEtAl2016}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{BrettEtAl2021, ThielEtAl2020, vanGompelEtAl2016, Brown2015, Zadka2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
document generation tools, like doxygen and sphinx\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind
review}%DIFDELCMD < ]%%%
\DIFdel{. %DIF < \cite{Michalski2021}
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Michalski2021}}\hskip0pt%DIFAUXCMD
...
software developers often use an agile philosophy \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{AckroydEtAl2008,
CarverEtAl2007, EasterbrookAndJohns2009, Segal2005, HeatonAndCarver2015}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{HeatonAndCarver2015, CarverEtAl2007, AckroydEtAl2008, EasterbrookAndJohns2009, Segal2005}}\hskip0pt%DIFAUXCMD
}\DIFaddend , or an
...
\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{Redacted for blind review}%DIFDELCMD < ] %%%
%DIF < Dong~\cite{Dong2021} 
\DIFdelend \DIFaddbegin \DIFadd{Dong (2021)~\mbox{%DIFAUXCMD
\cite{Dong2021} }\hskip0pt%DIFAUXCMD
%DIF > %DOUBLE BLIND
}\DIFaddend provides details on the specific approaches used by the interviewees.
...
software\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind review}%DIFDELCMD < ]
%DIFDELCMD < %%%
%DIF < \cite{Michalski2021} 
\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Michalski2021} }\hskip0pt%DIFAUXCMD
%DIF > %DOUBLE BLIND
}\DIFaddend and is apparently greater than the level of
...
reproducibility, and software scope determination.  \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind
review}%DIFDELCMD < ] %%%
%DIF < \cite{SmithEtAl2024} 
\DIFdelend \DIFaddbegin \DIFadd{Smith et al.\ (2024)~\mbox{%DIFAUXCMD
\cite{SmithEtAl2024} 
}\hskip0pt%DIFAUXCMD
}\DIFaddend adds lack of software experience, documentation and technical debt.  Our small
...
is not uncommon
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{PintoEtAl2018, PintoEtAl2016, WieseEtAl2019}}\hskip0pt%DIFAUXCMD
.
%DIF < DOUBLE BLIND \cite{SmithEtAl2024} [redacted for blind review]
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{SmithEtAl2024, WieseEtAl2019, PintoEtAl2018, PintoEtAl2016}}\hskip0pt%DIFAUXCMD
.
%DIF > DOUBLE BLIND
}\DIFaddend 
...
research software. This is a common complaint~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{GewaltigAndCannon2012, Goble2014, KaterbowAndFeulner2018}}\hskip0pt%DIFAUXCMD
. %DIF < , SmithEtAl2024 %[redacted for double blind review]
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{SmithEtAl2024, GewaltigAndCannon2012, Goble2014, KaterbowAndFeulner2018}}\hskip0pt%DIFAUXCMD
. %DIF > ,  %DOUBLE BLIND
}\DIFaddend Software output is not always counted when judging the academic excellence of
...
\cite{WieseEtAl2019}. 
%DIF < A similar pattern has been observed during interviews with
%DIF < LBM software developers [citation redacted for blind review].
%DIF < \cite{SmithEtAl2024}.
\DIFaddbegin \DIFadd{A similar pattern has been observed during interviews with
LBM software developers \mbox{%DIFAUXCMD
\cite{SmithEtAl2024}}\hskip0pt%DIFAUXCMD
. %DIF > %DOUBLE BLIND
}\DIFaddend 
...
\begin{itemize}
    ...
    \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Fowler2006}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite[p.\ 20]{HumbleAndFarley2010}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[p.\ 20]{HumbleAndFarley2010}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{Fowler2006}}\hskip0pt%DIFAUXCMD
...
Network (CRAN)\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted for blind review}%DIFDELCMD < ]%%%
\DIFdel{. %DIF < \cite{SmithEtAl2018_StatSoft}.
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{SmithEtAl2018_StatSoft}}\hskip0pt%DIFAUXCMD
. %DIF > %DOUBLE BLIND
}\DIFaddend 
...
averages around 30--35\% for the efficiency of defect removal \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{EbertAndJones2009,
Jones2008}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Jones2008, EbertAndJones2009}}\hskip0pt%DIFAUXCMD
}\DIFaddend . For research software, Kelly and Shepard \cite{KellyAndShepard2000}
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{WilsonEtAl2014, StewartEtAl2017, Storer2017}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Storer2017, WilsonEtAl2014, StewartEtAl2017}}\hskip0pt%DIFAUXCMD
}\DIFaddend ; however, specific guidelines
...
reproducible.
%% DOUBLE BLIND
%DIF < Smith et al.~\cite{SmithEtAl2016} 
\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{citation redacted}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{Smith et al.~\mbox{%DIFAUXCMD
\cite{SmithEtAl2016} }\hskip0pt%DIFAUXCMD
}\DIFaddend reports grading five software products by two reviewers.
...
approach to developing \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{other software (citation redacted)}%DIFDELCMD < ]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{LBM software \mbox{%DIFAUXCMD
\cite{SmithEtAl2024} 
}\hskip0pt%DIFAUXCMD
}\DIFaddend %% DOUBLE BLIND
%DIF < LBM software \cite{SmithEtAl2024} 
...
\begin{table*}[ht!]
    %\begin{center}
    \DIFaddbeginFL \caption{\DIFaddFL{Top performers for each quality (sorted by order of quality
    measurement)}}
    \DIFaddendFL \renewcommand{\arraystretch}{1.8}
    ...
    \DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Top performers for each quality (sorted by order of quality
    measurement)}} %DIFAUXCMD
\DIFdelendFL \label{topperformerstable}
    %\end{center}
\end{table*} 
...
\DIFdel{redacted for blind review}%DIFDELCMD < ]
%DIFDELCMD < %%%
\DIFdelend %% DOUBLE BLIND
%DIF <  We would like to thank Peter Michalski and Oluwaseun Owojaiye for fruitful
%DIF <  discussions on topics relevant to this paper.
\DIFaddbegin \DIFadd{We would like to thank Peter Michalski and Oluwaseun Owojaiye for fruitful
