...
\begin{longtable}{p{16cm}}
  ...
  Number of developers (number)\ad{It might be hard to decide. For example 3D Slicer has 171 people with commits to its repo, but on its website, 18 contributors(including people who are not developers) are acknowledged.}\\
  ...
  Is source code available? (\{yes, no\})\ad{It might be better to directly ask for a set of url, and the answer can be n/a if it's not open source, or unclear if it can't be determined. An example is as follows.}\\
  \ad{Example}Source code URL (\{set of url, n/a, unclear\})\\
  ...
  discussion board, sourceforge, google code, git, none, unclear\}) \ad{Does git mean GitHub and GitLab? Should we add BitBucket here?}\\
  Are the majority of identified bugs fixed? (\{yes, no$^*$, unclear\})\ad{It might be better to record a number here. An example is as follows.}\\
  \ad{Example}What is the percentage of identified issues that are closed? (percentage)\\
  ...
  (\{yes$^*$, no\})\\
  Are there code clones? (\{yes$^*$, no, unclear\})\ad{Might need to test some static analyzer.}\\
  Overall impression? (\{1 .. 10\})\\
  ...
  Are any portions of the software used by another package? (\{yes$^*$, no\})\ad{It seems hard to find an answer for some software.}\\
  ...
  Convincing evidence that portability has been achieved? (\{yes$^*$, no\})\ad{Seems like a good question to put in the questionnaire}\\
  Overall impression? (\{1 .. 10\})\\
