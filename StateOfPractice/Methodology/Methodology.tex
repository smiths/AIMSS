...
\ad{The ``tools'' here seems ambiguous. Does it mean the tools used by
developers for developing and maintaining the packages, or the dependencies
(such as libraries) used by software?}
...
\ad{From my understanding, we'll have two short lists. One is for the top
software packages for deeper AHP ranking, the other contains the software
packages whose developers are willing to take the survey. Should we separate
them here?}
\begin{enumerate}
...
\ad{For empirical measures, even if it's not a GitHub repo, a git repo is better
than a Mercurial or Subversion one. Should we describe our preference in such
order - GitHub > other git repos (in GitLab, BitBucket, SourceForge, etc.) >
other repos?}
...
observed from Git\ad{It's better to be ``GitHub'' here. GitLab, BitBucket and
SourceForge also mainly have git repos, but not providing all the metrics.}
repository metrics. The rest can be collected using freeware
